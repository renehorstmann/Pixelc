...
typedef struct ToolbarOld_Tool Tool;
...
static RoSingle *tool_append(ToolbarOld *self, float x, float y, const char *btn_file) {
    assert(self->L.tools_size < TOOLBAR_OLD_TOOL_MAX);
...
static bool pos_in_toolbar_old(const ToolbarOld *self, vec2 pos) {
...
ToolbarOld *toolbar_old_new(const Camera_s *camera,
                     ...
                     Animation *animation) {
    ToolbarOld *self = rhc_calloc(sizeof *self);
    ...
    for (int i = 0; i < TOOLBAR_OLD_MODES; i++) {
...
bool toolbar_old_pointer_event(ToolbarOld *self, ePointer_s pointer) {
    ...
    for (int i = 0; i < TOOLBAR_OLD_MODES; i++) {
        if (button_pressed(&self->L.modes[i]->rect, pointer)) {
            log_info("toolbar_old: mode %i", i);
            unpress(self->L.modes, TOOLBAR_OLD_MODES, i);
    ...
    if (button_clicked(&self->L.shape_minus->rect, pointer)) {
        log_info("toolbar_old: shape_minus");
    ...
    if (button_clicked(&self->L.shape_plus->rect, pointer)) {
        log_info("toolbar_old: shape_plus");
        int shape = self->brush_ref->RO.kernel_id;
        shape++;
    ...
    if (pointer.action == E_POINTER_DOWN && u_pose_aa_contains(self->L.color_drop.rect.pose, pointer.pos.xy)) {
        log_info("toolbar_old: secondary_color");
    ...
    if (self->show_selection_copy_cut) {
        if (button_toggled(&self->L.selection_copy.rect, pointer)) {
            log_info("toolbar_old: selection_copy");
        ...
        if (button_toggled(&self->L.selection_cut.rect, pointer)) {
            log_info("toolbar_old: selection_cut");
        ...
        for (int i = 0; i < 8; i++) {
            if (button_clicked(&self->L.selection_move[i].rect, pointer)) {
                ...
                if (!s) {
                    log_error("toolbar_old: selection_move failed, selection invalid (NULL)");
                    break;
                }
    ...
    if (self->show_selection_ok) {
        ...
        if(button_toggled(&self->L.selection_blend.rect, pointer)) {
            if(!self->selectionctrl_ref->selection) {
                log_error("toolbar_old: selection_blend failed, selection not valid (NULL)");
            } else {
