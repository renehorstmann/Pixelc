...
static void mode_pe(struct Tool *super, ePointer_s pointer, ToolRefs refs) {
    ...
    if(button_pressed(&self->ro.rect, pointer)) {
        ...
        refs.brush->mode = mode;
    } 
}

static bool mode_is_a(struct Tool *super, float dtime, ToolRefs refs) {
    ToolButton *self = (ToolButton *) super;
    enum brush_modes mode = *((enum brush_modes *) self->additional_data);
    button_set_pressed(&self->ro.rect, refs.brush->mode == mode);
    // always active
    return true;
}      
