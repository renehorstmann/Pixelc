...
/** dst = scalar */
static ucvec3 ucvec3_set(uchar scalar) {
    ucvec3 res;
...
/** dst = (ucvec3) v */
static ucvec3 ucvec3_cast_from_char(const char *v) {
    ucvec3 res;
    ucvecN_cast_into(res.v, v, 3);
    return res;
}
...
/** dst = a + b */
static ucvec3 ucvec3_add_vec_v(const uchar *vec_a, const uchar *vec_b) {
    return ucvec3_add_vec(UCVec3(vec_a), UCVec3(vec_b));
}


/** dst = a + b */
static ucvec3 ucvec3_add(ucvec3 vec_a, uchar scalar_b) {
    ucvec3 res;
    ucvecN_add(res.v, vec_a.v, scalar_b, 3);
    return res;
}
/** dst = a + b */
static ucvec3 ucvec3_add_v(const uchar *vec_a, uchar scalar_b) {
...
/** dst = a - b */
static ucvec3 ucvec3_sub_vec_v(const uchar *vec_a, const uchar *vec_b) {
    return ucvec3_sub_vec(UCVec3(vec_a), UCVec3(vec_b));
}


/** dst = a - b */
static ucvec3 ucvec3_sub(ucvec3 vec_a, uchar scalar_b) {
    ucvec3 res;
    ucvecN_sub(res.v, vec_a.v, scalar_b, 3);
    return res;
}
/** dst = a - b */
static ucvec3 ucvec3_sub_v(const uchar *vec_a, uchar scalar_b) {
...
/** dst = a * b */
static ucvec3 ucvec3_scale_vec_v(const uchar *vec_a, const uchar *vec_b) {
    return ucvec3_scale_vec(UCVec3(vec_a), UCVec3(vec_b));
}


/** dst = a * b */
static ucvec3 ucvec3_scale(ucvec3 vec_a, uchar scalar_b) {
    ucvec3 res;
    ucvecN_scale(res.v, vec_a.v, scalar_b, 3);
    return res;
}
/** dst = a * b */
static ucvec3 ucvec3_scale_v(const uchar *vec_a, uchar scalar_b) {
...
/** dst = a / b */
static ucvec3 ucvec3_div_vec_v(const uchar *vec_a, const uchar *vec_b) {
    return ucvec3_div_vec(UCVec3(vec_a), UCVec3(vec_b));
}


/** dst = a / b */
static ucvec3 ucvec3_div(ucvec3 vec_a, uchar scalar_b) {
    ucvec3 res;
    ucvecN_div(res.v, vec_a.v, scalar_b, 3);
    return res;
}
/** dst = a / b */
static ucvec3 ucvec3_div_v(const uchar *vec_a, uchar scalar_b) {
    return ucvec3_div(UCVec3(vec_a), scalar_b);
}


/** dst = x % y */
static ucvec3 ucvec3_mod(ucvec3 vec_x, uchar y) {
    ucvec3 res;
    ucvecN_mod(res.v, vec_x.v, y, 3);
    return res;
}
/** dst = x % y */
static ucvec3 ucvec3_mod_v(const uchar *vec_x, uchar y) {
...
/** dst = x % y */
static ucvec3 ucvec3_mod_vec_v(const uchar *vec_x, const uchar *vec_y) {
    return ucvec3_mod_vec(UCVec3(vec_x), UCVec3(vec_y));
}


/** dst = a < b ? a : b */
static ucvec3 ucvec3_min(ucvec3 vec_x, uchar y) {
    ucvec3 res;
    ucvecN_min(res.v, vec_x.v, y, 3);
    return res;
}
/** dst = a < b ? a : b */
static ucvec3 ucvec3_min_v(const uchar *vec_x, uchar y) {
...
/** dst = a < b ? a : b */
static ucvec3 ucvec3_min_vec_v(const uchar *vec_x, const uchar *vec_y) {
    return ucvec3_min_vec(UCVec3(vec_x), UCVec3(vec_y));
}


/** dst = a > b ? a : b */
static ucvec3 ucvec3_max(ucvec3 vec_x, uchar y) {
    ucvec3 res;
    ucvecN_max(res.v, vec_x.v, y, 3);
    return res;
}
/** dst = a > b ? a : b */
static ucvec3 ucvec3_max_v(const uchar *vec_x, uchar y) {
...
/** dst = a > b ? a : b */
static ucvec3 ucvec3_max_vec_v(const uchar *vec_x, const uchar *vec_y) {
    return ucvec3_max_vec(UCVec3(vec_x), UCVec3(vec_y));
}


/** dst = x < min ? min : (x > max ? max : x) */
static ucvec3 ucvec3_clamp(ucvec3 vec_x, uchar min, uchar max) {
    ucvec3 res;
    ucvecN_clamp(res.v, vec_x.v, min, max, 3);
    return res;
}
/** dst = x < min ? min : (x > max ? max : x) */
static ucvec3 ucvec3_clamp_v(const uchar *vec_x, uchar min, uchar max) {
...
/** dst = x < min ? min : (x > max ? max : x) */
static ucvec3 ucvec3_clamp_vec_v(const uchar *vec_x, const uchar *vec_min, const uchar *vec_max) {
...
/** dst = a * (1-t) + b * t */
static ucvec3 ucvec3_mix_v(const uchar *vec_a, const uchar *vec_b, float t) {
...
/** dst = a * (1-t) + b * t */
static ucvec3 ucvec3_mix_vec_v(const uchar *vec_a, const uchar *vec_b, const float *vec_t) {
    return ucvec3_mix_vec(UCVec3(vec_a), UCVec3(vec_b), vec_t);
}


/** dst = x < edge ? 0 : 1 */
static ucvec3 ucvec3_step(ucvec3 vec_x, uchar edge) {
    ucvec3 res;
    ucvecN_step(res.v, vec_x.v, edge, 3);
    return res;
}
/** dst = x < edge ? 0 : 1 */
static ucvec3 ucvec3_step_v(const uchar *vec_x, uchar edge) {
...
/** dst = x < edge ? 0 : 1 */
static ucvec3 ucvec3_step_vec_v(const uchar *vec_x, const uchar *vec_edge) {
    return ucvec3_step_vec(UCVec3(vec_x), UCVec3(vec_edge));
}


/** returns vec[0] + vec[1] + ... + vec[n-1] */
static uchar ucvec3_sum(ucvec3 vec) {
    return ucvecN_sum(vec.v, 3);
}
/** returns vec[0] + vec[1] + ... + vec[n-1] */
static uchar ucvec3_sum_v(const uchar *vec) {
    return ucvec3_sum(UCVec3(vec));
}


/** returns a dot b */
static uchar ucvec3_dot(ucvec3 vec_a, ucvec3 vec_b) {
    return ucvecN_dot(vec_a.v, vec_b.v, 3);
}
/** returns a dot b */
static uchar ucvec3_dot_v(const uchar *vec_a, const uchar *vec_b) {
...
/** returns ||vec||_3 */
static float ucvec3_norm_v(const uchar *vec) {
...
/** returns length of a vector, see ucvecN_norm. Just here to match glsl */
static float ucvec3_length_v(const uchar *vec) {
...
/** returns norm(b-a) */
static float ucvec3_distance_v(const uchar *vec_a, const uchar *vec_b) {
    return ucvec3_distance(UCVec3(vec_a), UCVec3(vec_b));
}


/** dst = a < b */
static bvec3 ucvec3_less_than(ucvec3 vec_a, uchar b) {
    bvec3 res;
    ucvecN_less_than(res.v, vec_a.v, b, 3);
    return res;
}
/** dst = a < b */
static bvec3 ucvec3_less_than_v(const uchar *vec_a, uchar b) {
...
/** dst = a < b */
static bvec3 ucvec3_less_than_vec_v(const uchar *vec_a, const uchar *vec_b) {
    return ucvec3_less_than_vec(UCVec3(vec_a), UCVec3(vec_b));
}


/** dst = a <= b */
static bvec3 ucvec3_less_than_equal(ucvec3 vec_a, uchar b) {
    bvec3 res;
    ucvecN_less_than_equal(res.v, vec_a.v, b, 3);
    return res;
}
/** dst = a <= b */
static bvec3 ucvec3_less_than_equal_v(const uchar *vec_a, uchar b) {
...
/** dst = a <= b */
static bvec3 ucvec3_less_than_equal_vec_v(const uchar *vec_a, const uchar *vec_b) {
    return ucvec3_less_than_equal_vec(UCVec3(vec_a), UCVec3(vec_b));
}


/** dst = a > b */
static bvec3 ucvec3_greater_than(ucvec3 vec_a, uchar b) {
    bvec3 res;
    ucvecN_greater_than(res.v, vec_a.v, b, 3);
    return res;
}
/** dst = a > b */
static bvec3 ucvec3_greater_than_v(const uchar *vec_a, uchar b) {
...
/** dst = a > b */
static bvec3 ucvec3_greater_than_vec_v(const uchar *vec_a, const uchar *vec_b) {
    return ucvec3_greater_than_vec(UCVec3(vec_a), UCVec3(vec_b));
}


/** dst = a >= b */
static bvec3 ucvec3_greater_than_equal(ucvec3 vec_a, uchar b) {
    bvec3 res;
    ucvecN_greater_than_equal(res.v, vec_a.v, b, 3);
    return res;
}
/** dst = a >= b */
static bvec3 ucvec3_greater_than_equal_v(const uchar *vec_a, uchar b) {
...
/** dst = a >= b */
static bvec3 ucvec3_greater_than_equal_vec_v(const uchar *vec_a, const uchar *vec_b) {
    return ucvec3_greater_than_equal_vec(UCVec3(vec_a), UCVec3(vec_b));
}


/** dst = a == b */
static bvec3 ucvec3_equal(ucvec3 vec_a, uchar b) {
    bvec3 res;
    ucvecN_equal(res.v, vec_a.v, b, 3);
    return res;
}
/** dst = a == b */
static bvec3 ucvec3_equal_v(const uchar *vec_a, uchar b) {
...
/** dst = a == b */
static bvec3 ucvec3_equal_vec_v(const uchar *vec_a, const uchar *vec_b) {
    return ucvec3_equal_vec(UCVec3(vec_a), UCVec3(vec_b));
}


/** dst = a != b */
static bvec3 ucvec3_not_equal(ucvec3 vec_a, uchar b) {
    bvec3 res;
    ucvecN_not_equal(res.v, vec_a.v, b, 3);
    return res;
}
/** dst = a != b */
static bvec3 ucvec3_not_equal_v(const uchar *vec_a, uchar b) {
...
/** dst = a != b */
static bvec3 ucvec3_not_equal_vec_v(const uchar *vec_a, const uchar *vec_b) {
...
/** dst = a x b */
static ucvec3 ucvec3_cross_v(const uchar *vec_a, const uchar *vec_b) {
