...
/** dst = scalar */
static cvec2 cvec2_set(char scalar) {
    cvec2 res;
...
/** dst = -vec */
static cvec2 cvec2_neg_v(const char *vec) {
...
/** dst = a + b */
static cvec2 cvec2_add_vec_v(const char *vec_a, const char *vec_b) {
    return cvec2_add_vec(CVec2(vec_a), CVec2(vec_b));
}


/** dst = a + b */
static cvec2 cvec2_add(cvec2 vec_a, char scalar_b) {
    cvec2 res;
    cvecN_add(res.v, vec_a.v, scalar_b, 2);
    return res;
}
/** dst = a + b */
static cvec2 cvec2_add_v(const char *vec_a, char scalar_b) {
...
/** dst = a - b */
static cvec2 cvec2_sub_vec_v(const char *vec_a, const char *vec_b) {
    return cvec2_sub_vec(CVec2(vec_a), CVec2(vec_b));
}


/** dst = a - b */
static cvec2 cvec2_sub(cvec2 vec_a, char scalar_b) {
    cvec2 res;
    cvecN_sub(res.v, vec_a.v, scalar_b, 2);
    return res;
}
/** dst = a - b */
static cvec2 cvec2_sub_v(const char *vec_a, char scalar_b) {
...
/** dst = a * b */
static cvec2 cvec2_scale_vec_v(const char *vec_a, const char *vec_b) {
    return cvec2_scale_vec(CVec2(vec_a), CVec2(vec_b));
}


/** dst = a * b */
static cvec2 cvec2_scale(cvec2 vec_a, char scalar_b) {
    cvec2 res;
    cvecN_scale(res.v, vec_a.v, scalar_b, 2);
    return res;
}
/** dst = a * b */
static cvec2 cvec2_scale_v(const char *vec_a, char scalar_b) {
...
/** dst = a / b */
static cvec2 cvec2_div_vec_v(const char *vec_a, const char *vec_b) {
    return cvec2_div_vec(CVec2(vec_a), CVec2(vec_b));
}


/** dst = a / b */
static cvec2 cvec2_div(cvec2 vec_a, char scalar_b) {
    cvec2 res;
    cvecN_div(res.v, vec_a.v, scalar_b, 2);
    return res;
}
/** dst = a / b */
static cvec2 cvec2_div_v(const char *vec_a, char scalar_b) {
...
/** dst = abs(x) */
static cvec2 cvec2_abs_v(const char *vec_x) {
...
/** dst = x > 0 ? 1 : (x < 0 ? -1 : 0) */
static cvec2 cvec2_sign_v(const char *vec_x) {
    return cvec2_sign(CVec2(vec_x));
}


/** dst = x % y */
static cvec2 cvec2_mod(cvec2 vec_x, char y) {
    cvec2 res;
    cvecN_mod(res.v, vec_x.v, y, 2);
    return res;
}
/** dst = x % y */
static cvec2 cvec2_mod_v(const char *vec_x, char y) {
...
/** dst = x % y */
static cvec2 cvec2_mod_vec_v(const char *vec_x, const char *vec_y) {
    return cvec2_mod_vec(CVec2(vec_x), CVec2(vec_y));
}


/** dst = a < b ? a : b */
static cvec2 cvec2_min(cvec2 vec_x, char y) {
    cvec2 res;
    cvecN_min(res.v, vec_x.v, y, 2);
    return res;
}
/** dst = a < b ? a : b */
static cvec2 cvec2_min_v(const char *vec_x, char y) {
...
/** dst = a < b ? a : b */
static cvec2 cvec2_min_vec_v(const char *vec_x, const char *vec_y) {
    return cvec2_min_vec(CVec2(vec_x), CVec2(vec_y));
}


/** dst = a > b ? a : b */
static cvec2 cvec2_max(cvec2 vec_x, char y) {
    cvec2 res;
    cvecN_max(res.v, vec_x.v, y, 2);
    return res;
}
/** dst = a > b ? a : b */
static cvec2 cvec2_max_v(const char *vec_x, char y) {
...
/** dst = a > b ? a : b */
static cvec2 cvec2_max_vec_v(const char *vec_x, const char *vec_y) {
    return cvec2_max_vec(CVec2(vec_x), CVec2(vec_y));
}


/** dst = x < min ? min : (x > max ? max : x) */
static cvec2 cvec2_clamp(cvec2 vec_x, char min, char max) {
    cvec2 res;
    cvecN_clamp(res.v, vec_x.v, min, max, 2);
    return res;
}
/** dst = x < min ? min : (x > max ? max : x) */
static cvec2 cvec2_clamp_v(const char *vec_x, char min, char max) {
...
/** dst = x < min ? min : (x > max ? max : x) */
static cvec2 cvec2_clamp_vec_v(const char *vec_x, const char *vec_min, const char *vec_max) {
...
/** dst = a * (1-t) + b * t */
static cvec2 cvec2_mix_v(const char *vec_a, const char *vec_b, float t) {
...
/** dst = a * (1-t) + b * t */
static cvec2 cvec2_mix_vec_v(const char *vec_a, const char *vec_b, const float *vec_t) {
    return cvec2_mix_vec(CVec2(vec_a), CVec2(vec_b), vec_t);
}


/** dst = x < edge ? 0 : 1 */
static cvec2 cvec2_step(cvec2 vec_x, char edge) {
    cvec2 res;
    cvecN_step(res.v, vec_x.v, edge, 2);
    return res;
}
/** dst = x < edge ? 0 : 1 */
static cvec2 cvec2_step_v(const char *vec_x, char edge) {
...
/** dst = x < edge ? 0 : 1 */
static cvec2 cvec2_step_vec_v(const char *vec_x, const char *vec_edge) {
...
/** returns vec[0] + vec[1] + ... + vec[n-1] */
static int cvec2_sum_v(const char *vec) {
...
/** returns a dot b */
static int cvec2_dot_v(const char *vec_a, const char *vec_b) {
...
/** returns ||vec||_2 */
static float cvec2_norm_v(const char *vec) {
...
/** returns ||vec||_p */
static float cvec2_norm_p_v(const char *vec, float p) {
...
/** returns ||vec||_1 */
static int cvec2_norm_1_v(const char *vec) {
...
/** returns ||vec||_inf */
static int cvec2_norm_inf_v(const char *vec) {
...
/** returns length of a vector, see cvecN_norm. Just here to match glsl */
static float cvec2_length_v(const char *vec) {
...
/** returns norm(b-a) */
static float cvec2_distance_v(const char *vec_a, const char *vec_b) {
    return cvec2_distance(CVec2(vec_a), CVec2(vec_b));
}


/** dst = a < b */
static bvec2 cvec2_less_than(cvec2 vec_a, char b) {
    bvec2 res;
    cvecN_less_than(res.v, vec_a.v, b, 2);
    return res;
}
/** dst = a < b */
static bvec2 cvec2_less_than_v(const char *vec_a, char b) {
...
/** dst = a < b */
static bvec2 cvec2_less_than_vec_v(const char *vec_a, const char *vec_b) {
    return cvec2_less_than_vec(CVec2(vec_a), CVec2(vec_b));
}


/** dst = a <= b */
static bvec2 cvec2_less_than_equal(cvec2 vec_a, char b) {
    bvec2 res;
    cvecN_less_than_equal(res.v, vec_a.v, b, 2);
    return res;
}
/** dst = a <= b */
static bvec2 cvec2_less_than_equal_v(const char *vec_a, char b) {
...
/** dst = a <= b */
static bvec2 cvec2_less_than_equal_vec_v(const char *vec_a, const char *vec_b) {
    return cvec2_less_than_equal_vec(CVec2(vec_a), CVec2(vec_b));
}


/** dst = a > b */
static bvec2 cvec2_greater_than(cvec2 vec_a, char b) {
    bvec2 res;
    cvecN_greater_than(res.v, vec_a.v, b, 2);
    return res;
}
/** dst = a > b */
static bvec2 cvec2_greater_than_v(const char *vec_a, char b) {
...
/** dst = a > b */
static bvec2 cvec2_greater_than_vec_v(const char *vec_a, const char *vec_b) {
    return cvec2_greater_than_vec(CVec2(vec_a), CVec2(vec_b));
}


/** dst = a >= b */
static bvec2 cvec2_greater_than_equal(cvec2 vec_a, char b) {
    bvec2 res;
    cvecN_greater_than_equal(res.v, vec_a.v, b, 2);
    return res;
}
/** dst = a >= b */
static bvec2 cvec2_greater_than_equal_v(const char *vec_a, char b) {
...
/** dst = a >= b */
static bvec2 cvec2_greater_than_equal_vec_v(const char *vec_a, const char *vec_b) {
    return cvec2_greater_than_equal_vec(CVec2(vec_a), CVec2(vec_b));
}


/** dst = a == b */
static bvec2 cvec2_equal(cvec2 vec_a, char b) {
    bvec2 res;
    cvecN_equal(res.v, vec_a.v, b, 2);
    return res;
}
/** dst = a == b */
static bvec2 cvec2_equal_v(const char *vec_a, char b) {
...
/** dst = a == b */
static bvec2 cvec2_equal_vec_v(const char *vec_a, const char *vec_b) {
    return cvec2_equal_vec(CVec2(vec_a), CVec2(vec_b));
}


/** dst = a != b */
static bvec2 cvec2_not_equal(cvec2 vec_a, char b) {
    bvec2 res;
    cvecN_not_equal(res.v, vec_a.v, b, 2);
    return res;
}
/** dst = a != b */
static bvec2 cvec2_not_equal_v(const char *vec_a, char b) {
...
/** dst = a != b */
static bvec2 cvec2_not_equal_vec_v(const char *vec_a, const char *vec_b) {
