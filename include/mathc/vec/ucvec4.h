...
/** dst = scalar */
static ucvec4 ucvec4_set(uint8_t scalar) {
    ucvec4 res;
...
/** dst = (ucvec4) v */
static ucvec4 ucvec4_cast_from_char(const int8_t *v) {
    ucvec4 res;
    ucvecN_cast_into(res.v, v, 4);
    return res;
}
...
/** dst = a + b */
static ucvec4 ucvec4_add_vec_v(const uint8_t *vec_a, const uint8_t *vec_b) {
    return ucvec4_add_vec(UCVec4(vec_a), UCVec4(vec_b));
}


/** dst = a + b */
static ucvec4 ucvec4_add(ucvec4 vec_a, uint8_t scalar_b) {
    ucvec4 res;
    ucvecN_add(res.v, vec_a.v, scalar_b, 4);
    return res;
}
/** dst = a + b */
static ucvec4 ucvec4_add_v(const uint8_t *vec_a, uint8_t scalar_b) {
...
/** dst = a - b */
static ucvec4 ucvec4_sub_vec_v(const uint8_t *vec_a, const uint8_t *vec_b) {
    return ucvec4_sub_vec(UCVec4(vec_a), UCVec4(vec_b));
}


/** dst = a - b */
static ucvec4 ucvec4_sub(ucvec4 vec_a, uint8_t scalar_b) {
    ucvec4 res;
    ucvecN_sub(res.v, vec_a.v, scalar_b, 4);
    return res;
}
/** dst = a - b */
static ucvec4 ucvec4_sub_v(const uint8_t *vec_a, uint8_t scalar_b) {
...
/** dst = a * b */
static ucvec4 ucvec4_scale_vec_v(const uint8_t *vec_a, const uint8_t *vec_b) {
    return ucvec4_scale_vec(UCVec4(vec_a), UCVec4(vec_b));
}


/** dst = a * b */
static ucvec4 ucvec4_scale(ucvec4 vec_a, uint8_t scalar_b) {
    ucvec4 res;
    ucvecN_scale(res.v, vec_a.v, scalar_b, 4);
    return res;
}
/** dst = a * b */
static ucvec4 ucvec4_scale_v(const uint8_t *vec_a, uint8_t scalar_b) {
...
/** dst = a / b */
static ucvec4 ucvec4_div_vec_v(const uint8_t *vec_a, const uint8_t *vec_b) {
    return ucvec4_div_vec(UCVec4(vec_a), UCVec4(vec_b));
}


/** dst = a / b */
static ucvec4 ucvec4_div(ucvec4 vec_a, uint8_t scalar_b) {
    ucvec4 res;
    ucvecN_div(res.v, vec_a.v, scalar_b, 4);
    return res;
}
/** dst = a / b */
static ucvec4 ucvec4_div_v(const uint8_t *vec_a, uint8_t scalar_b) {
...
/** dst = x % y */
static ucvec4 ucvec4_mod(ucvec4 vec_x, uint8_t y) {
    ucvec4 res;
    ucvecN_mod(res.v, vec_x.v, y, 4);
    return res;
}
/** dst = x % y */
static ucvec4 ucvec4_mod_v(const uint8_t *vec_x, uint8_t y) {
...
/** dst = x % y */
static ucvec4 ucvec4_mod_vec_v(const uint8_t *vec_x, const uint8_t *vec_y) {
    return ucvec4_mod_vec(UCVec4(vec_x), UCVec4(vec_y));
}


/** dst = a < b ? a : b */
static ucvec4 ucvec4_min(ucvec4 vec_x, uint8_t y) {
    ucvec4 res;
    ucvecN_min(res.v, vec_x.v, y, 4);
    return res;
}
/** dst = a < b ? a : b */
static ucvec4 ucvec4_min_v(const uint8_t *vec_x, uint8_t y) {
...
/** dst = a < b ? a : b */
static ucvec4 ucvec4_min_vec_v(const uint8_t *vec_x, const uint8_t *vec_y) {
    return ucvec4_min_vec(UCVec4(vec_x), UCVec4(vec_y));
}


/** dst = a > b ? a : b */
static ucvec4 ucvec4_max(ucvec4 vec_x, uint8_t y) {
    ucvec4 res;
    ucvecN_max(res.v, vec_x.v, y, 4);
    return res;
}
/** dst = a > b ? a : b */
static ucvec4 ucvec4_max_v(const uint8_t *vec_x, uint8_t y) {
...
/** dst = a > b ? a : b */
static ucvec4 ucvec4_max_vec_v(const uint8_t *vec_x, const uint8_t *vec_y) {
    return ucvec4_max_vec(UCVec4(vec_x), UCVec4(vec_y));
}


/** dst = x < min ? min : (x > max ? max : x) */
static ucvec4 ucvec4_clamp(ucvec4 vec_x, uint8_t min, uint8_t max) {
    ucvec4 res;
    ucvecN_clamp(res.v, vec_x.v, min, max, 4);
    return res;
}
/** dst = x < min ? min : (x > max ? max : x) */
static ucvec4 ucvec4_clamp_v(const uint8_t *vec_x, uint8_t min, uint8_t max) {
...
/** dst = x < min ? min : (x > max ? max : x) */
static ucvec4 ucvec4_clamp_vec_v(const uint8_t *vec_x, const uint8_t *vec_min, const uint8_t *vec_max) {
...
/** dst = a * (1-t) + b * t */
static ucvec4 ucvec4_mix_v(const uint8_t *vec_a, const uint8_t *vec_b, float t) {
...
/** dst = a * (1-t) + b * t */
static ucvec4 ucvec4_mix_vec_v(const uint8_t *vec_a, const uint8_t *vec_b, const float *vec_t) {
    return ucvec4_mix_vec(UCVec4(vec_a), UCVec4(vec_b), vec_t);
}


/** dst = x < edge ? 0 : 1 */
static ucvec4 ucvec4_step(ucvec4 vec_x, uint8_t edge) {
    ucvec4 res;
    ucvecN_step(res.v, vec_x.v, edge, 4);
    return res;
}
/** dst = x < edge ? 0 : 1 */
static ucvec4 ucvec4_step_v(const uint8_t *vec_x, uint8_t edge) {
...
/** dst = x < edge ? 0 : 1 */
static ucvec4 ucvec4_step_vec_v(const uint8_t *vec_x, const uint8_t *vec_edge) {
    return ucvec4_step_vec(UCVec4(vec_x), UCVec4(vec_edge));
}


/** returns vec[0] + vec[1] + ... + vec[n-1] */
static uint8_t ucvec4_sum(ucvec4 vec) {
    return ucvecN_sum(vec.v, 4);
}
/** returns vec[0] + vec[1] + ... + vec[n-1] */
static uint8_t ucvec4_sum_v(const uint8_t *vec) {
    return ucvec4_sum(UCVec4(vec));
}


/** returns a dot b */
static uint8_t ucvec4_dot(ucvec4 vec_a, ucvec4 vec_b) {
    return ucvecN_dot(vec_a.v, vec_b.v, 4);
}
/** returns a dot b */
static uint8_t ucvec4_dot_v(const uint8_t *vec_a, const uint8_t *vec_b) {
...
/** returns ||vec||_4 */
static float ucvec4_norm_v(const uint8_t *vec) {
...
/** returns length of a vector, see ucvecN_norm. Just here to match glsl */
static float ucvec4_length_v(const uint8_t *vec) {
...
/** returns norm(b-a) */
static float ucvec4_distance_v(const uint8_t *vec_a, const uint8_t *vec_b) {
    return ucvec4_distance(UCVec4(vec_a), UCVec4(vec_b));
}


/** dst = a < b */
static bvec4 ucvec4_less_than(ucvec4 vec_a, uint8_t b) {
    bvec4 res;
    ucvecN_less_than(res.v, vec_a.v, b, 4);
    return res;
}
/** dst = a < b */
static bvec4 ucvec4_less_than_v(const uint8_t *vec_a, uint8_t b) {
...
/** dst = a < b */
static bvec4 ucvec4_less_than_vec_v(const uint8_t *vec_a, const uint8_t *vec_b) {
    return ucvec4_less_than_vec(UCVec4(vec_a), UCVec4(vec_b));
}


/** dst = a <= b */
static bvec4 ucvec4_less_than_equal(ucvec4 vec_a, uint8_t b) {
    bvec4 res;
    ucvecN_less_than_equal(res.v, vec_a.v, b, 4);
    return res;
}
/** dst = a <= b */
static bvec4 ucvec4_less_than_equal_v(const uint8_t *vec_a, uint8_t b) {
...
/** dst = a <= b */
static bvec4 ucvec4_less_than_equal_vec_v(const uint8_t *vec_a, const uint8_t *vec_b) {
    return ucvec4_less_than_equal_vec(UCVec4(vec_a), UCVec4(vec_b));
}


/** dst = a > b */
static bvec4 ucvec4_greater_than(ucvec4 vec_a, uint8_t b) {
    bvec4 res;
    ucvecN_greater_than(res.v, vec_a.v, b, 4);
    return res;
}
/** dst = a > b */
static bvec4 ucvec4_greater_than_v(const uint8_t *vec_a, uint8_t b) {
...
/** dst = a > b */
static bvec4 ucvec4_greater_than_vec_v(const uint8_t *vec_a, const uint8_t *vec_b) {
    return ucvec4_greater_than_vec(UCVec4(vec_a), UCVec4(vec_b));
}


/** dst = a >= b */
static bvec4 ucvec4_greater_than_equal(ucvec4 vec_a, uint8_t b) {
    bvec4 res;
    ucvecN_greater_than_equal(res.v, vec_a.v, b, 4);
    return res;
}
/** dst = a >= b */
static bvec4 ucvec4_greater_than_equal_v(const uint8_t *vec_a, uint8_t b) {
...
/** dst = a >= b */
static bvec4 ucvec4_greater_than_equal_vec_v(const uint8_t *vec_a, const uint8_t *vec_b) {
    return ucvec4_greater_than_equal_vec(UCVec4(vec_a), UCVec4(vec_b));
}


/** dst = a == b */
static bvec4 ucvec4_equal(ucvec4 vec_a, uint8_t b) {
    bvec4 res;
    ucvecN_equal(res.v, vec_a.v, b, 4);
    return res;
}
/** dst = a == b */
static bvec4 ucvec4_equal_v(const uint8_t *vec_a, uint8_t b) {
...
/** dst = a == b */
static bvec4 ucvec4_equal_vec_v(const uint8_t *vec_a, const uint8_t *vec_b) {
    return ucvec4_equal_vec(UCVec4(vec_a), UCVec4(vec_b));
}


/** dst = a != b */
static bvec4 ucvec4_not_equal(ucvec4 vec_a, uint8_t b) {
    bvec4 res;
    ucvecN_not_equal(res.v, vec_a.v, b, 4);
    return res;
}
/** dst = a != b */
static bvec4 ucvec4_not_equal_v(const uint8_t *vec_a, uint8_t b) {
...
/** dst = a != b */
static bvec4 ucvec4_not_equal_vec_v(const uint8_t *vec_a, const uint8_t *vec_b) {
...
/** dst = a x b , dst.w = 0 */
static ucvec4 ucvec4_cross_v(const uint8_t *vec_a, const uint8_t *vec_b) {
